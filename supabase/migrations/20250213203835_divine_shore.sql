/*
  ...
  1. New Tables
    - chat_threads: Stores chat conversations between patients and providers
      - id (uuid, primary key)
      - type (text)
      - metadata (jsonb)
    ...
    - chat_participants: Stores thread participants
  ...
  2. Security
    ...
    - Add policies to ensure users can only access their own chats
    - Add policies to ensure providers can only chat with their patients

  3. Functions
    - get_or_create_chat_thread: Creates or returns existing thread
    - mark_messages_read: Marks messages as read
    - get_unread_message_count: Gets count of unread messages
*/
...
CREATE POLICY "chat_threads_access"
...
CREATE POLICY "chat_attachments_access"
