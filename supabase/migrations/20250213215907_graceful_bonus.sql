/*
  ...
  2. Functions
    - `toggle_habit_completion`: Toggle completion status for a habit on a specific date
    - `get_habit_stats`: Get statistics for a habit including streaks and completion rates
...
BEGIN
  ...
  DROP POLICY IF EXISTS "habits_user_access" ON public.habits;
  
  -- Drop existing functions
  DROP FUNCTION IF EXISTS toggle_habit_completion(uuid, date);
...
CREATE POLICY "habits_access_policy"
ON public.habits
FOR ALL
USING (
  auth.uid() = user_id
...
CREATE INDEX IF NOT EXISTS idx_habits_user_lookup 
ON habits(user_id);

CREATE INDEX IF NOT EXISTS idx_habits_updated_lookup 
