...
BEGIN
  ...
  DROP POLICY IF EXISTS "protocols_provider_access" ON public.protocols;
  DROP POLICY IF EXISTS "protocols_patient_access" ON public.protocols;
  DROP POLICY IF EXISTS "protocol_tasks_access" ON public.protocols;
  DROP POLICY IF EXISTS "protocol_attachments_access" ON public.protocol_attachments;
...
CREATE POLICY "protocols_provider_manage"
ON public.protocols
FOR ALL
USING (
...
CREATE POLICY "protocols_patient_view"
ON public.protocols
FOR SELECT
USING (
  auth.uid() = patient_id
...
CREATE POLICY "protocol_tasks_inherit"
...
CREATE POLICY "protocol_attachments_inherit"
...
BEGIN
  WITH protocol_data AS (
    ...
    GROUP BY p.id, p.title, p.description, p.type, p.status, p.provider_id, 
             pr.name, p.start_date, p.end_date, p.notes, p.created_at, p.updated_at
    ORDER BY 
      CASE p.status 
        WHEN 'active' THEN 1
